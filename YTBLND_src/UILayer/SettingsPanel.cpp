    ...
    , m_nav(std::move(nav))
{
    ...
    themePanel->SetBackgroundColour(UIColors::Surface());
    ...
    // Theme Options, populated dynamically from the ThemeMap
    wxArrayString choices;
    for (auto const& [name, palette] : UIColors::ThemeMap) {
        choices.Add(name);
    }
...
void SettingsPanel::OnThemeChanged(wxCommandEvent& evt) {
    ...
    // Broadcast the event so logic-heavy panels can react
